...
**Out of scope:** feature detection (Phase 5), any agent backend (Phase 4), live transcript streaming (Phase 4 — the SSE seam is reserved, not built), spend display (Phase 6, OBS-05), budget enforcement (Phase 6), the dashboard (Phase 14), forge operations (Phase 5).
...
- **OpenAPI document for the HTTP API** — declined in D-20 for this phase. If the Phase 14 dashboard or third-party clients make a published contract worthwhile, it is additive over the REST-ish surface.
