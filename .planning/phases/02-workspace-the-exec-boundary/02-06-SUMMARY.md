...
| T-2-27 | accept | **Accepted, but NOT on the grounds recorded here originally.** This row read "Only durability is weaker, and it is loud", and the 02-SECURITY audit falsified it against the code this same plan shipped. The four shared controls are real — the stub uses the same `run`, `buildChildEnv`, scratch `HOME` and containment guard, and the contract suite proves all four. The fifth is not shared: `stub/backend.ts` calls `run(execSpec, scratchHome.path, log)` with no worker identity, so it resolves `worker-user-unset` and runs agent children **undropped** on a Linux deployment where `worktree/backend.ts` would have dropped them. Security posture is therefore weaker too, by exactly one control. It stays accepted at `low` because nothing in v1 selects a backend at all (`workspaceRegistry().resolve()` has no production caller — Phase 3 owns that wiring), the gap is one named control rather than a class of them, and WORK-05's banner fires on the first non-dropped exec in the process. See `02-06-PLAN.md`'s amended register entry for the full argument and **D-2-07-2** for the tracked fix, which is owed *before* backend selection exists. Revisit also when the container backend makes the id set heterogeneous in its guarantees. |
