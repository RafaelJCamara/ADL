...
describe('VerdictSchema — CORE-01', () => {
  it.each(OUTCOMES)(
    'parses the minimal payload for outcome "%s"',
    (outcome) => {
  ...
  it('rejects a payload with no outcome field at all', () => {
    expect(
      VerdictSchema.safeParse({ summary: 'no outcome here' }).success,
    ).toBe(false);
  });

  it.each(OUTCOMES)(
    'consumesRound is %s only for send_back — checked for "%s"',
    (outcome) => {
