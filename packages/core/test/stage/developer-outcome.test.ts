...
describe('DeveloperOutcomeSchema — self-approval is unrepresentable (D-05)', () => {
  it('declares exactly three members, and none of them is a pass', () => {
    expect(DEVELOPER_OUTCOME_KINDS).toEqual(['committed', 'dispute', 'blocked']);
  ...
  it('rejects a pass in every shape it could be attempted', () => {
    ...
    const attempts: unknown[] = [
      ...
      { outcome: 'pass', summary: 'looks good', checked: [{ kind: 'criterion', id: 'AC-1' }] },
    ];

    for (const attempt of attempts) {
      expect(DeveloperOutcomeSchema.safeParse(attempt).success, JSON.stringify(attempt)).toBe(false);
    }
  });

  it('accepts committed with a commit sha', () => {
    const result = DeveloperOutcomeSchema.safeParse({ kind: 'committed', sha: 'a1b2c3d4e5'.repeat(4) });
    expect(result.success).toBe(true);
  });

  it('rejects committed without a plausible sha', () => {
    expect(DeveloperOutcomeSchema.safeParse({ kind: 'committed' }).success).toBe(false);
    expect(DeveloperOutcomeSchema.safeParse({ kind: 'committed', sha: '' }).success).toBe(false);
    expect(DeveloperOutcomeSchema.safeParse({ kind: 'committed', sha: 'not a sha' }).success).toBe(
      false,
    );
  });
  ...
  it('accepts blocked with a reason, and rejects one without', () => {
    expect(
      DeveloperOutcomeSchema.safeParse({ kind: 'blocked', reason: 'the database migration will not apply' })
        .success,
    ).toBe(true);
    expect(DeveloperOutcomeSchema.safeParse({ kind: 'blocked' }).success).toBe(false);
    expect(DeveloperOutcomeSchema.safeParse({ kind: 'blocked', reason: '' }).success).toBe(false);
  });

  it('accepts a complete dispute', () => {
    expect(DeveloperOutcomeSchema.safeParse({ kind: 'dispute', dispute: COMPLETE_DISPUTE }).success).toBe(
      true,
    );
  });
});
...
describe('DisputeSchema — structure is what makes a dispute triageable (D-06)', () => {
  ...
  it('rejects a target that names neither a fingerprint nor a stage id', () => {
    expect(DisputeSchema.safeParse({ ...COMPLETE_DISPUTE, target: {} }).success).toBe(false);
    expect(
      DisputeSchema.safeParse({ ...COMPLETE_DISPUTE, target: { kind: 'finding' } }).success,
    ).toBe(false);
    expect(DisputeSchema.safeParse({ ...COMPLETE_DISPUTE, target: { kind: 'stage' } }).success).toBe(
      false,
    );
  });
  ...
  it('rejects a fingerprint that is not 64 characters', () => {
    expect(
      DisputeSchema.safeParse({ ...COMPLETE_DISPUTE, target: { kind: 'finding', fingerprint: 'abc' } })
        .success,
    ).toBe(false);
  });
  ...
  it('rejects a missing or empty argument', () => {
    ...
    expect(DisputeSchema.safeParse({ ...COMPLETE_DISPUTE, argument: '' }).success).toBe(false);
  });
  ...
  it('makes a dispute malformed rather than partially valid when a field is missing', () => {
    ...
    expect(DeveloperOutcomeSchema.safeParse({ kind: 'dispute', dispute: noRef }).success).toBe(false);
  });
});

describe('round accounting — the honest exit is never charged for', () => {
  it('reports zero rounds consumed for all three kinds', () => {
    expect(DEVELOPER_OUTCOME_ROUND_COST).toEqual({ committed: 0, dispute: 0, blocked: 0 });
  ...
  it('covers every declared kind, so a fourth cannot be added without a decision', () => {
    expect(Object.keys(DEVELOPER_OUTCOME_ROUND_COST).sort()).toEqual([...DEVELOPER_OUTCOME_KINDS].sort());
  });
});
