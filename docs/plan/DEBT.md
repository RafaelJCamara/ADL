...
| **D-5-13-1** (M05) | `worker-entry/stage-runner.ts`, `workspace/worktree/lifecycle.ts` | **A workspace does not survive the stage that created it, so a gate would judge a tree with none of the developer's work in it.** `createProductionStageRunner` calls `workspace.destroy()` in its `finally` — which reclaims the worktree *and* deletes the `adl/*` branch — and `createWorktree` deliberately **refuses** to attach to an existing worktree or branch ("a second worker sharing one worktree with a running agent is a data-loss path", WORK-04). One worker runs per stage, so the gate M05 step 5.14 adds would call `backend.create()` and branch from `baseRef`, seeing none of the commit it is supposed to judge. **The same gap breaks crash recovery, today, independently of any gate:** `dispatchOnce` preserves `workspace_handle` across a recovery dispatch precisely so recovery re-attaches, but nothing attaches — a worker killed before its `finally` ran leaves a worktree that the recovery dispatch's `create()` then throws on (`provider_error`, retryable, forever). **Not reached by 5.13's own tests** because its scripted pipeline worker builds no workspace and its production refusal for a non-zero stage index returns *before* one is created. **Proposed shape:** `WorkspaceBackend.attach(spec)` — the method `.planning/research/ARCHITECTURE.md` §1 already names ("recovery re-attaches to the existing workspace rather than recloning — `WorkspaceBackend.attach(handle)` exists for exactly this") and that was never built — plus a decision about who destroys a workspace once the feature leaves the loop (the GC sweep already collects terminal features, and `escalated` deliberately survives it). Note this widens a **one-way port**: `WorkspaceBackend` is republished through `@adl/plugin-sdk` (D-01). **Owner: M05 step 5.14**, the first step with a gate that needs a workspace | 🟠 |
...
| ~~**D-5-11-1**~~ (M05) | `publish/role-rounds.ts` | **Partly closed by M05 step 5.13, and its premise was wrong.** The item said writing a real `RoundOutcome` into `rounds.outcome_json` would stop a prior round losing its commit sha. It could not have: `RoundOutcome` has no field for a commit. What 5.13's round loop *does* write is the round's real result, and `role-rounds.ts` now reads it — a finished round folds away as `send_back — 3 findings` or `escalate — <reason>` rather than a bare kind (`test/publish/role-rounds.test.ts`). **What remains is only the sha**, and it is now diagnosed rather than open: its durable home is a `rounds.head_sha` column, which is a migration, and one with a second consumer already waiting — a gate needs the diff between the base and exactly that commit. **Owner: M05 step 5.14** | 🟡 |
