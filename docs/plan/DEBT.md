...
| **D-5-11-1** (M05) | `publish/role-rounds.ts` | **A round's commit sha survives only while that round is the newest.** The sticky comment is re-derived in full from `rounds`/`stage_attempts`/`verdicts`/`findings` every round (FORGE-06, 5.11), and the sha is in none of them — it arrives on the `developer_outcome: committed` event as a `RoundNote` and is written into that round's body once. Republishing during round 2 renders round 1 from the database alone, so round 1's fold loses its sha. **Reproduced** and asserted as the current behaviour in `packages/manager/test/publish/sticky-comment.test.ts` ("edits the SAME comment on a second round"). Not fixed here deliberately: the durable home is `rounds.outcome_json`, whose writer is the round loop, and fabricating a `RoundOutcome` to hold a sha would corrupt the column that step is built on. **Proposed shape:** 5.13 writes the real `RoundOutcome` at the round-completion write site; `deriveHeadline`/the body builder then read it and the `RoundNote` seam either narrows to "the round still in flight" or disappears. **Owner: M05 step 5.13.** The existing assertion is the tripwire — it fails the moment round outcomes start being written, forcing the question rather than letting the comment stay quietly lossy | 🟡 |
...
| **Wall-clock scenario tests flake under heavy parallelism** | Observed during M05 step 5.11: `pnpm vitest run --project core --project forge-github --project manager` (three projects at once — *not* how CI runs them) failed once on `test/prompt/determinism.test.ts` and once on `test/scenario/detect-restart-reconciliation.test.ts`, a **different** test each run, both passing in isolation immediately after. Both fork real worker processes and wait on `waitUntil` deadlines, so contention, not logic, is the plausible cause. `pnpm test` — the CI-equivalent, per-package command — was green on both runs. Not investigated further: the failing invocation is not one CI or the documented workflow uses. **Risk if wrong:** these deadlines are tuned to one developer machine, so a slower CI runner could turn this into a real intermittent red | 🟡 |
