...
- [ ] One forge conformance suite runs against real GitHub *and* a Dockerised Gitea in CI,
...
- [ ] **14.6** — Confirm no base-interface capability is unhonourable on Gitea. If one is,
      the *interface* is wrong, not Gitea.

## Notes

**Ordering is deliberate.** GitLab is second because it is genuinely *different* and forces
