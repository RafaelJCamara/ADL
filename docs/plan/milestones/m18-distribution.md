...
- [ ] The maintainer runs ADL in **observe-only mode** and sees exactly what it *would* do,
...
- [ ] **18.6** — systemd unit + Dockerfile. **The manager *is* the worker supervisor; the
...
narrate what it *would* do, touching nothing, will say yes far more often than one asked
to grant repo write access on trust.

ADL is installed into *someone else's* repository — so extension points, configuration
