...
Each entry says what was decided and *why*. Where a decision was taken against contrary
...
**The unit of work is a *feature* — a described behaviour — not a ticket or a diff.**
...
measured in *review time saved* — but the delivered artefact is more code to review, so if
...
**Dogfooding is a hard gate *partway through*, not the finish line.**
...
⚠️ *Taken against research advice.* All four research passes independently recommended
...
gap — a delegated-loop CLI *and* an owned-loop raw API. Claude plus an OpenAI CLI proves
...
config, credentials, round and budget accounting. **Forge *reads* belong to the manager too.**
...
backend can never work. Enforced by lint, with the exemption count *measured* by a test.
...
**The adapter layer is *two* ports, not one.**
...
> of the time, with Claude-family models specifically preferring to *modify the tests* —
...
**Kysely with hand-written SQL migrations — *not* Drizzle.**
Drizzle's stable release is still pre-1.0 with an RC pending; choosing it would schedule a
known breaking migration into a nights-and-weekends project. ⚠️ *Note: `.claude/CLAUDE.md`'s
research section still recommends Drizzle. It is stale on this point — **Kysely is settled,
and no Drizzle migration phase exists or should be added.***
